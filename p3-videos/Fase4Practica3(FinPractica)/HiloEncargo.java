...
public class HiloEncargo implements Runnable  {//Mejor poner que implementa Runnable aunque tambien vale poner extends Thread pero es mas facil con Runnable
	...
	public void run() {
		...
		for (int j = 0; j < peticion.getEncargo().getVideos().size(); j++) {
			primero = new Trabajo(peticion.getEncargo().getVideos().get(j));
			
			aux.getAndIncrement();
		    primero =cola.encolarTrabajo(primero);
		    //colaAux.put(trabajo.hashCode(), peticion.getEncargo().getVideos().get(pos));
		    
		    //Debemos hacer otra comprobacion final y es que hay un caso en el que pueda fallar la 
		    //transcodificacion y devuelva algo vacio; es decir, el objeto Video devuelto al transcodificar
		    //no devuelva nada
		    if(primero.getVideoNuevo() == null) {
		    	//En el caso que esto ocurra debemos avisar y lo vamos a hacer utilizando el cerrojo
		    	//y la condicion descritas
		    	Alertador objetoAlerta = new Alertador(cerrojo ,condicion);
		    	primero.setAlertador(objetoAlerta);
		    	  numTrEncargo.incrementAndGet(); //Equivale a incrementar valores del contador de los trabajos
		    }
		...
		numTrEncargo.set(0);//Le damos el valor 0 al empezar, no deberia haber problemas de cambio
		                    //de valor de todos modos si nos queremos asegurar poniendo esta linea 
		                    //verificamos su valor 0
		...
		try {
			...
			for (Trabajo trbjo : list)
			{
				if(trbjo.getVideoNuevo() == null) {
					...
					numTrEncargo.incrementAndGet();//Equivale a decrementar valores del contador de los trabajos
				}
			}
	        ...
	        while(numTrEncargo.get()>0) {//Obtiene el valor de la variable contador
	        	...
	        	numTrEncargo.decrementAndGet();//Equivale a decrementar valores del contador de los trabajos
	        }
